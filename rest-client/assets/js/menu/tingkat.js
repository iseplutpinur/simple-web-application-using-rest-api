function tingkat(alert = null){
					...
					<div class="row">
						<div class="container-fluid">
					  		<div class="card">
								    ...
								    <h5 class="my-0 mr-md-auto font-weight-normal">Data Tingkat</h5>
								    <!-- modal bootom -->
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="formModalTambahLabel">Tambah Data Tingkat</h5>
	...
	const modalUbah   = `
		...
		<div class="modal fade" id="formModalUbah" tabindex="-1" role="dialog" aria-labelledby="formModalUbahLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="formModalUbahLabel">Edit Data Tingkat</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
	...
	const modalDetail = `
		...
		<div class="modal fade" id="modalDetail" tabindex="-1" role="dialog" aria-labelledby="modalDetailLabel" aria-hidden="true">
			<div class="modal-dialog modal-xl" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="modalDetailLabel">Detail Tingkat</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
	...
	const script      = `
		...
		function hapusData(data){
			...
			$('#btn-delete').attr('onclick', 'tingkat_delete('+ data.dataset.id + ')');
		}
		...
		$('#btn-ubah').on('click', ()=>{
			tingkat_ubah({id: $('#id-ubah').val(),nama: $('#nama-ubah').val()});
		});
		</script>`;
	const tingkat_render = (data) => {
	...
	const getData = ()=>{
		...
		$.ajax({
			...
			data: {
				'get': 'tingkat'},
			success: result => {
				if (result.response) tingkat_render(result);
...
function tingkat_tambah(data){
	cekKoneksi = false;
	const act = () => {
		mainLoading();
		$.ajax({
			url: base_url("?post=tingkat"),
			type: 'post',
			dataType: 'json',
			data: {
				'nama': data,
			...
			success: result => {
				if (result.response) tingkat({icon: 'success', text: "berhasil menambahkan data"});
				else tingkat({icon: 'falied', text: "gagal menambahkan data"});
...
function tingkat_ubah(data){
	cekKoneksi = false;
	const act = () => {
		mainLoading();
		$.ajax({
			url: base_url("?put=tingkat"),
			type: 'post',
			dataType: 'json',
			data: {
				'nama': data.nama,
			...
			success: result => {
				if (result.response) tingkat({icon: 'success', text: "berhasil mengubah data"});
				else tingkat({icon: 'falied', text: "gagal mengubah data"});
...
function tingkat_delete(id){
	...
	const act = () => {		
		...
		$.ajax({
			...
			success: result => {
				if (result.response) {
					tingkat({icon: 'success', text: "berhasil menghapus data"});
					cekKoneksi = true;					
				} else tingkat({icon: 'falied', text: "gagal menghapus data"});
			}
		});
	};
	setTimeout(act,500);
